...
function Home() {
    ...
    useEffect(()=>{
        const fetchData=async()=>{
         try {
            const response=await axios.get(`https://internshipbackend-teng.onrender.com/api/internship`)
   ...
   const filterInternShips=internshipData.filter((item)=>
    ...
    const handleSlideIntern=(direction)=>{
        const contianer2=document.getElementById("container2");
        const step=100;
        if (direction==='left') {
            setCurrentSlide((preveSlibe)=>(preveSlibe>0 ?preveSlibe-1:0))
        }
        ...
        sideScrollIntern(contianer2, direction, 25, step, 10)
    }
  return(
    <>
...
<div className="infoys">
    <div className="info-intern">
        <div className="mt-16">
            <h1 className='text-center text-3xl font-bold'>Latest Internships of internarea</h1>
        </div>
            ...
            <div className="Internship-Info flex">
                {
                    ...
                    <>
                     ...
                     <hr className='mt-5'/>
                     <p className='mt-2'> <i class='bi bi-geo-alt '></i>{data.location}</p>
                     ...
                     <p className='mt-1'><i class='bi bi-calendar-fill'></i>{data.Duration}</p>
                      <div className='more flex justify-between mt-6'>
                        <span className='bg-slate-200
                         text-slate-400 w-20
                          rounded-sm h-7 text-center'>Internship</span>
                         
                         <Link to={`detailInternship?q=${data._id}`}> <span className='text-blue-400 mt-2'>
...
<a href="/register" id='buttons' class="flex items-center  bg-white h-9 justify-center mt-4 text-white rounded-lg shadow-md hover:bg-gray-100">
               <div class="px-4 py-3 ">
                   <svg class="h-6 w-5" viewBox="0 0 40 40">
               ...
               <p class="px-4 py-1 w-5/6 text-center text-sm text-gray-600 font-bold ">Sign in with Google</p>
  </a>
  <a to="/register">
