...
function Job() {
    ...
    useEffect(()=>{
        const fetchData=async()=>{
         try {
            const response=await axios.get(`https://internshipbackend-teng.onrender.com/api/job`)
...
""}`} onClick={()=>setselectedCategory("Data Science")}>Data Science</span>
            ...
            <div className="Internship-Info flex">
                {
                    ...
                    <>
                     ...
                     <hr className='mt-5'/>
                     <p className='mt-3'> <i class='bi bi-geo-alt '></i>{data.location}</p>
                     ...
                     <p className='mt-1'><i class='bi bi-calendar-fill'></i>{data.Experience}</p>
                      <div className='more flex justify-between mt-6'>
                        <span className='bg-slate-200
                         text-slate-400 w-20
                          rounded-sm h-7 text-center'>Job</span>
                          <Link to={`detailJob?q=${data._id}`}><span className='text-blue-400 mt-2'>
 ...
 <button className='back' onClick={()=>handleSlide('left')}> <i className='bi bi-chevron-left'
  id='sideBack'></i></button>
<button  className="next" onClick={()=>handleSlide('right')}> <i className='bi bi-chevron-right'
 id='slide'></i></button>
</div>
    </div>
</div>
